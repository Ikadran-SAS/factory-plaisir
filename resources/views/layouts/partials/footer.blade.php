{{-- ── FOOTER PRINCIPAL SEO ── --}}
<footer class="footer">
    <div class="container">
        <div class="footer-grid">

            {{-- Colonne 1: Brand + Contact --}}
            <div class="footer-col">
                <p class="footer-logo-text">Factory &amp; Co</p>
                <p class="footer-tagline">Delicious since 1989</p>
                <address class="footer-address">
                    <strong>14 Rue du Danube</strong><br>
                    CC Val d'Europe<br>
                    77700 Serris, France<br><br>
                    <a href="tel:+33164632889" style="color:var(--pink-dark);font-weight:600">+33 1 64 63 28 89</a><br>
                    <a href="mailto:contact@factoryandco.com" style="color:var(--pink-dark);font-weight:600">contact@factoryandco.com</a><br><br>
                    <small style="color:#888">Parking gratuit • RER A Val d'Europe</small>
                </address>
            </div>

            {{-- Colonne 2: Navigation --}}
            <div class="footer-col">
                <h3>Navigation</h3>
                <ul>
                    <li><a href="{{ route('home') }}">Accueil</a></li>
                    <li><a href="{{ route('concept') }}">Notre concept</a></li>
                    <li><a href="{{ route('menu.index') }}">La carte</a></li>
                    <li><a href="{{ route('specialites') }}">Nos spécialités</a></li>
                    <li><a href="{{ route('avis') }}">Avis clients</a></li>
                </ul>
            </div>

            {{-- Colonne 3: Notre carte --}}
            <div class="footer-col">
                <h3>Notre carte</h3>
                <ul>
                    <li><a href="{{ route('menu.index') }}#burgers">Smash Burgers</a></li>
                    <li><a href="{{ route('menu.index') }}#bagels">Bagels New-York</a></li>
                    <li><a href="{{ route('menu.index') }}#bowls">Bowls &amp; salades</a></li>
                    <li><a href="{{ route('menu.index') }}#cheesecake">Cheesecakes</a></li>
                    <li><a href="{{ route('click-collect') }}">Click &amp; Collect</a></li>
                </ul>
            </div>

            {{-- Colonne 4: Infos pratiques --}}
            <div class="footer-col">
                <h3>Infos pratiques</h3>
                <ul>
                    <li><a href="{{ route('faq') }}">FAQ complète</a></li>
                    <li><a href="{{ route('contact') }}">Nous contacter</a></li>
                    <li><a href="{{ route('sitemap.html') }}">Plan du site</a></li>
                    <li><a href="#">Horaires d'ouverture</a></li>
                </ul>
            </div>

            {{-- Colonne 5: Légal &amp; Autres --}}
            <div class="footer-col">
                <h3>Légal &amp; Services</h3>
                <ul>
                    <li><a href="{{ route('mentions-legales') }}">Mentions légales</a></li>
                    <li><a href="{{ route('confidentialite') }}">Politique de confidentialité</a></li>
                    <li><a href="{{ route('conditions-utilisation') }}">Conditions d'utilisation</a></li>
                    <li><a href="https://www.factoryandco.com/recrutement/" target="_blank" rel="noopener" class="external-link">Recrutement <svg class="external-icon" fill="currentColor" viewBox="0 0 24 24"><path d="M19 19H5V5h7V3H5a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14c1.1 0 2-.9 2-2v-7h-2v7zM14 3v2h3.59l-9.83 9.83l1.41 1.41L19 6.41V10h2V3h-7z"/></svg></a></li>
                    <li><a href="https://www.factoryandco.com/devenez-franchise/" target="_blank" rel="noopener" class="external-link">Devenez franchise <svg class="external-icon" fill="currentColor" viewBox="0 0 24 24"><path d="M19 19H5V5h7V3H5a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14c1.1 0 2-.9 2-2v-7h-2v7zM14 3v2h3.59l-9.83 9.83l1.41 1.41L19 6.41V10h2V3h-7z"/></svg></a></li>
                </ul>
            </div>
        </div>

        {{-- Footer bottom --}}
        <div class="footer-bottom">
            <div class="footer-bottom-left">
                <p class="footer-copy">
                    © {{ date('Y') }} Factory &amp; Co Val d'Europe. Tous droits réservés.
                </p>
            </div>
            <div class="footer-social">
                <a href="https://www.instagram.com/factoryandco/" target="_blank" rel="noopener" aria-label="Suivez-nous sur Instagram">
                    <svg fill="currentColor" viewBox="0 0 24 24"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/></svg>
                </a>
                <a href="https://www.facebook.com/factoryandco/" target="_blank" rel="noopener" aria-label="Suivez-nous sur Facebook">
                    <svg fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                </a>
            </div>
        </div>
    </div>
</footer>
